...
const Card = ({ data, index, hoveredIndex, isDragging, setHoveredIndex }) => {
    ...
    return (
        ...
        >
            ...
            <div className="w-[50vw] sm:w-[23.229vw] sm:h-[16.979vw] absolute  left-[1.25vw] sm:top-0">
                ...
                {/* Title */}
                <div
                    className={`sm:pl-0 pl-2 absolute left-0 top-10 sm:top-[3.073vw] transition-colors duration-500 ease-in-out ${isHovered ? "text-[#00aff1]" : "opacity-70 text-white"} text-[2.824vw] sm:text-[1.25vw] font-semibold sm:leading-[1.759vw]`}
                >
                    {data.title}
                </div>
                ...
                {/* Description */}
                <div
                    className={`sm:pl-0 pl-2 absolute left-0 top-16 sm:top-[5.729vw] transition-colors duration-500 ease-in-out ${isHovered ? "text-white" : "opacity-70 text-white"} text-[2.824vw] sm:text-[1.25vw] font-normal`}
                >
                    {data.description}
                </div>
            </div>
            ...
            {Array.from({ length: window.innerWidth <= 640 ? 70 : 35 }).map((_, i) => (
                <div
                    key={i}
                    ...
                    style={{
                        ...
                        top: window.innerWidth <= 640 ? "50vw" : "19.757vw", // 50vw for mobile, 19.757vw for larger screens
                        backgroundColor: isFilled ? "#00aff1" : "rgba(255, 255, 255, 0.2)", // Fill based on whether step is completed
