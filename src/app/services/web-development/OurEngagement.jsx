...
const Card = ({ title, content, isMobile }) => (
    <div className={`bg-[#131d22] text-white p-6 rounded-[0.833vw] transition-colors duration-300 hover:bg-white hover:text-black flex flex-col ${isMobile ? 'h-auto mb-6' : 'h-full'} group`}>
        <h2 className={`${isMobile ? 'text-[5vw]' : 'text-[2.5vw]'} font-medium leading-loose group-hover:text-black`}>{title}</h2>
        <div className="flex flex-col gap-[0.625vw]">
            {content.map((paragraph, index) => (
                <p key={index} className={`${isMobile ? 'w-full text-[3.5vw]' : 'w-[39.167vw] text-[1.25vw]'} text-white/70 font-normal group-hover:text-black/70`}>
        ...
        {isMobile ? (
            <div className="flex justify-between mt-4">
                <button className="text-blue-400 text-[3vw]">See More</button>
                <button className="bg-gray-700 text-white text-[3vw] py-2 px-4 rounded">Contact Us</button>
            </div>
        ) : (
            <div className="mt-auto self-end w-[3.802vw] h-[4.01vw] relative bg-white/10 rounded-[0.729vw] shadow flex justify-center items-center group-hover:bg-black/10">
                <FaChevronRight className="w-[1.25vw] h-[1.25vw] group-hover:text-black" />
            </div>
        )}
    </div>
);
        ...
        <div className='bg-[#00111A]'>
            ...
            <div className='hidden lg:block md:block px-[5.208vw] py-[5.508vw]'>
                <div className='flex justify-center items-center mb-[4.896vw]'>
                    <h5 className='text-white text-[2.5vw] font-bold leading-[2.083vw]'>Our Engagement Models</h5>
                </div>
            ...
            <div className='sm:hidden px-6 py-8'>
                <div className='flex justify-center items-center mb-6'>
                    <h5 className='text-white text-[6vw] font-bold leading-tight text-center'>Our Engagement Models</h5>
                </div>
                {cards.map((card, index) => (
